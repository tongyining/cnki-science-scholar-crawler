...
(function() {
    ...
    function isSupported() {
        ...
        for (const site of SUPPORTED_SITES) {
            console.log("检查是否支持自动填入")
            if (url.includes(site.domain) && url.includes(site.pathPattern)) {
                console.log("支持")
                return true;
            }else{
                console.log("不支持")
        ...
        // 后备：只要存在知网搜索框且域名包含关键字
        if ((url.includes('cnki.ccki.top') || url.includes('kns-cnki-net-443.wvpn.sjlib.cn')) && document.querySelector('#txt_search')) {
            return true;
        }
        return false;
    }
    ...
    console.log('[辅助面板] 当前页面支持，正在注入辅助面板...');
    ...
    header.style.cssText = `
        ...
        align-items: center;
    `;
    header.innerHTML = '<span>📚 批量搜索助手</span><span id="close-panel" style="cursor:pointer;">✕</span>';
    ...
    content.innerHTML = `
        ...
        <div style="margin-bottom: 10px;">
            <button id="format-btn" style="background:#0078d7; color:white; border:none; padding:6px 12px; border-radius:4px; cursor:pointer;">✨ 格式化生成搜索按钮</button>
            <button id="clear-btn" style="background:#ccc; border:none; padding:6px 12px; border-radius:4px; margin-left:8px; cursor:pointer;">🗑 清空列表</button>
        </div>
        <div id="title-list-area" style="max-height: 400px; overflow-y: auto; border-top:1px solid #eee; padding-top:8px;">
            <div style="color:#999; text-align:center;">点击“格式化”后显示标题列表</div>
        </div>
    `;
    ...
    document.getElementById('clear-btn')?.addEventListener('click', () => {
        ...
        if (area) area.innerHTML = '<div style="color:#999; text-align:center;">列表已清空</div>';
    ...
    document.getElementById('format-btn')?.addEventListener('click', () => {
        ...
        lines.forEach((title, idx) => {
            ...
            // 截断过长显示
            if (title.length > 40) titleSpan.textContent = title.substr(0, 38) + '...';
            ...
            searchBtn.addEventListener('click', (e) => {
                ...
                if (!searchInput) {
                    alert('未找到搜索框 #txt_search，请确认页面是否正常');
                    return;
                }
                // 清空并输入
                searchInput.value = '';
                ...
                if (!processedTitles.has(title)) {
                    ...
                    console.log(`[辅助面板] 已标记: ${title}`);
                } else {
                    console.log(`[辅助面板] 标题已处理过: ${title}`);
                }
                // 可选：自动焦点到搜索按钮（让用户手动点击）
                // 不做自动点击搜索按钮，让用户手动点网站的搜索按钮
            });
